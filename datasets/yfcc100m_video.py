...
def make_dataset(yfcc_root, yfcc_results, target_classes, n_samples_for_each_video, topk):
    dataset = []
    res = pd.read_pickle(yfcc_results)
    video_list = []
    label_list = []
    count = 0
    for i, c in enumerate(target_classes):

        video_list.extend(res[c][:topk])
        #print(res['segment'][c])
        label_list.extend([i] * topk)
    ...
    print('Load %d yfcc videos' % (count))
    return dataset
...
class YFCC100M(data.Dataset):
    ...
    def __init__(self,
        ...
        self.data = make_dataset(yfcc_root, yfcc_results, target_classes, n_samples_for_each_video, topk)
